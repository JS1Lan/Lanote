...
- 当前真实代码/Git 仓库仍是 `E:\桌面\播放器软件\Lanote\resources\app`。
- 当前版本是 `v1.1.0` 纯净安装发布线；本轮已从当前可信源码重新生成并发布 `dist/Lanote-1.1.0-Setup.exe`。
- GitHub 仓库已公开：`https://github.com/XxHuberrr/Lanote`
- `v1.1.0` Release：`https://github.com/XxHuberrr/Lanote/releases/tag/v1.1.0`
...
- 用户很在意视觉质感，尤其讨厌“默认白框”“太素”“没设计感”。Lanote 视觉方向偏黑色、玻璃、舞台、音乐可视化。
...
- 安装器默认安装目录优先使用 `D:\Lanote`，并创建桌面快捷方式。
...
- `Lanote-<version>-Setup.exe`
- `Lanote-<version>-Setup.exe.blockmap`
- `Lanote-<from>-to-<to>.patch.json`
...
- 已执行 `npm run build:win`，第一次被旧代理 `127.0.0.1:26001` 拦截，切到 `127.0.0.1:10808` 后打包成功。产物：`dist/Lanote-1.1.0-Setup.exe`、`.blockmap`、`Lanote-1.1.0-SHA256SUMS.txt`。
...
- 发布 `v1.0.4` 到 GitHub：`https://github.com/XxHuberrr/Lanote/releases/tag/v1.0.4`。
- 本次发布包含安装包 `Lanote-1.0.4-Setup.exe`、`latest.yml`、blockmap，以及 `1.0.0/1.0.1/1.0.2/1.0.3 -> 1.0.4` 四个快速补丁 JSON。
...
- 生成并上传 `Lanote-0.9.10-to-0.9.11.patch.json`。
...
- 已生成 `dist/Lanote-0.9.12-Setup.exe`、`dist/Lanote-0.9.12-Setup.exe.blockmap`、`dist/latest.yml`。
- 已生成轻量快速补丁 `dist/Lanote-0.9.11-to-0.9.12.patch.json`，补丁只覆盖 `package.json`、`package-lock.json`、`public/index.html`，用于已安装 `0.9.11` 的用户快速更新视觉和封面粒子修复。
- 已创建并核对 GitHub Release `v0.9.12`：`https://github.com/XxHuberrr/Lanote/releases/tag/v0.9.12`，远端包含安装包、blockmap、`latest.yml` 和 `0.9.11-to-0.9.12` 快速补丁。
...
- 用户随后明确要求上传 GitHub：已升级到 `0.9.13`，更新 `CHANGELOG.md` 和 `RELEASE.md`，生成 `dist/Lanote-0.9.12-to-0.9.13.patch.json` 快速补丁，并重新构建 `dist/Lanote-0.9.13-Setup.exe`、`dist/Lanote-0.9.13-Setup.exe.blockmap`、`dist/latest.yml`。
- 已推送提交 `4d9044a Prepare Lanote 0.9.13 release` 到 `origin/main`，并创建 GitHub Release `v0.9.13`：`https://github.com/XxHuberrr/Lanote/releases/tag/v0.9.13`。远端资产包含安装包、blockmap、`latest.yml` 和 `0.9.12-to-0.9.13` 快速补丁。
